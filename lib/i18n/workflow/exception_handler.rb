    ...
    class ExceptionHandler < I18n::ExceptionHandler
      ...
      def call(exception, locale, key, options)
        if exception.is_a?(I18n::MissingTranslation)
          ...
          locale = keys.shift
          last_key = keys.pop
          # priit commented out: keys = [locale] + keys.map { |s| "#{s}_scope".to_sym } + [last_key]
          ...
          # priit commented out: interpolations = options.symbolize_keys.except(*(I18n::RESERVED_KEYS + [:rescue_format]))
          # priit commmented out: keys.last.to_s.tr('_', ' ').capitalize + ' ' + interpolations.values.join(' ')
        else
          super
        end
      end
      ...
      def store_missing_translations(locale=nil)
        ...
        file.write(missing_translations_to_hash(locale).deep_stringify_keys.deep_merge(current_missing_translations).to_yaml)
        file.close
